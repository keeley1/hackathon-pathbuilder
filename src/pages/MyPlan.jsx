...
const MyPlan = () => {
  ...
  // Show only incomplete tasks as upcoming
        ...
        <div>
          ...
          <div className="bg-primary-light rounded-lg p-4 mb-4">
            <ul className="space-y-2">
              {upcomingTasks.length === 0 ? (
                <li className="text-text-muted">No upcoming tasks!</li>
              ) : (
                upcomingTasks.map((task) => (
                  <li key={task.id} className="flex justify-between text-text">
                    <span className="truncate max-w-[8rem]">{task.name}</span>
                    <span className="truncate max-w-[12rem] text-text-muted text-xs">{task.description}</span>
                  </li>
                ))
              )}
            </ul>
          ...
          <div className="bg-accent-light rounded-lg p-4">
            <div className="w-full bg-primary-light rounded-full h-3 mb-2">
              <div className="bg-primary h-3 rounded-full" style={{ width: `${overallProgress}%` }}></div>
            </div>
            <div className="text-xs text-text-muted">{overallProgress}% complete</div>
          </div>
        </div>
      </div>
    </div>
